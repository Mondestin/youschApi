...
use App\Repositories\Attendance\TeacherAttendanceRepositoryInterface;
...
class TeacherAttendanceController extends Controller
{
    public function __construct(
        private TeacherAttendanceRepositoryInterface $attendanceRepository
    ) {}
    ...
    public function index(Request $request): JsonResponse
    {
        $filters = $request->only([
            'teacher_id', 'class_id', 'subject_id', 'date', 'status', 'term', 'academic_year'
        ]);
    ...
    public function store(Request $request): JsonResponse
    {
        ...
        return response()->json([
            'success' => true,
            'message' => 'Teacher attendance record created successfully',
            'data' => $attendance->load(['teacher', 'class', 'subject', 'lab', 'timetable'])
        ], 201);
    }
    ...
    public function update(Request $request, TeacherAttendance $attendance): JsonResponse
    {
        ...
        return response()->json([
            'success' => false,
            'message' => 'Failed to update teacher attendance record'
        ], 500);
    }
    ...
    public function byTeacher(Request $request, int $teacherId): JsonResponse
    {
        ...
        $attendance = $this->attendanceRepository->getAttendanceByTeacher($teacherId, $filters);
    ...
    public function bulkCreate(Request $request): JsonResponse
    {
        $validator = Validator::make($request->all(), [
            ...
            'attendance_data.*.teacher_id' => 'required|exists:teachers,id',
    ...
    public function bulkUpdate(Request $request): JsonResponse
    {
        $validator = Validator::make($request->all(), [
            'attendance_data' => 'required|array|min:1',
            'attendance_data.*.id' => 'required|exists:teacher_attendance,id',
            'attendance_data.*.status' => 'sometimes|in:present,absent,late',
            'attendance_data.*.remarks' => 'nullable|string|max:500',
        ]);
    ...
    public function teacherSummary(Request $request, int $teacherId): JsonResponse
    {
        ...
        $summary = $this->attendanceRepository->getTeacherAttendanceSummary(
            $teacherId,
            $request->start_date,
            $request->end_date
        );
